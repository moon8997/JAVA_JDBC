...
public class DQL_select_02 {
	...
	public static void main(String[] args) {
      ...
      // PreparedStatement pstmt 은 Connection conn(연결한 오라클 서버)에 전송할 SQL문(편지)을 전달할 객체(우편배달부)이다. 
      ...
      // ResultSet rs 는 select 되어진 결과물이 저장되어지는 곳.
         ...
         */
			...
			while(rs.next()) {
	            ...
	            */
		/*
				int no = rs.getInt("no"); // "no" 는 select 해온 컬러명이다.
				String name = rs.getString("NAME");
				String msg = rs.getString("MSG");
				String writeday = rs.getString("writeday");
		*/
				// 또는
				int no = rs.getInt(1); // 1은 select 해온 첫번째컬럼명이다.
