...
public class Procedure_select_many_02 {
	 ...
	 // ResultSet rs 은 select 되어진 결과물이 저장되어지는 곳.  
		...
		try {
			...
			*/
			...
			cstmt.setString(1, addr_search); // 숫자 1은 프로시저 파라미터중 첫번째 파라미터인 IN 모드의 ? 를 말한다.
			...
			// 그래서 2번째 파라미터에 저장되어진 정보를 꺼내오도록 한다.
			...
			if(cnt == 0) {
				System.out.println(">> 검색하신 주소 "+addr_search+"에 거주하는 학생은 없습니다. << ");
			}
			else {
